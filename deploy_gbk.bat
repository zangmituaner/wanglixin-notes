...
if not exist ".git" (
    ...
    git init
    git config user.name "wanglixin"
    git config user.email "wanglixin@example.com"
    git add index.html style.css script.js deploy_gbk.bat README.md
    git commit -m "Initialize notes project"
    echo [成功] Git 仓库已初始化
) else (
    echo [信息] Git 仓库已存在
    git add index.html style.css script.js deploy_gbk.bat README.md
    git commit -m "Update notes content" 2>nul || echo [信息] 无内容需要提交
)
